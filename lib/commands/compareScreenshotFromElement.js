...
function retrieveCache(client, selector, customName, readDirCallback) {
  ...
  let filepath = path.join(client.options.screenshotsPath, fixedName);
  if (client.globals.customScreenshotsPath) {
    filepath = path.join(client.globals.customScreenshotsPath, fixedName);
  }
  const options = client.options.dayguard || {};
  let customFilePath = filepath;
  if (client.globals.customStaticScreenshotsPath) {
    customFilePath = path.join(client.globals.customStaticScreenshotsPath, fixedName);
  }
  const cache = util.cache(options, customFilePath, fixedSelector);
  if (cache && client.globals.customStaticScreenshotsPath) {
...
module.exports.command = function compareScreenshotFromElement(selector, customName, done) {
  ...
  if (this.globals.customStaticScreenshotsPath) {
    return this
      .perform((api, cb) => {
